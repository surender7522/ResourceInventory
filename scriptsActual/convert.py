...
import copy
'''
Clean up and create pyd
'''
directory = os.fsencode("OpenAPI/")
files = glob.glob('output_pyd_new/*')
# for f in files:
#     os.remove(f)
for file in os.listdir(directory):
    filename = os.fsdecode(file)
    if filename.endswith(".yaml") and ("SliceNrm") in filename:
        print(filename)
        subprocess.run(["datamodel-codegen","--input", "OpenAPI/"+filename, "--output", "output_pyd_new/"+filename.split(".yaml")[0]+".py"])
